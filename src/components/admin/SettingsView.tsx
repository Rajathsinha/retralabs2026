                ...
                {section.toggles.map((t) => (
                  <label key={t.key} className="flex items-center justify-between cursor-pointer">
                    ...
                    <button
                      ...
                      className={`relative w-11 h-6 rounded-full transition-colors ${(settings as any)[t.key] ? 'bg-blue-500' : 'bg-slate-200'}`}
                    >
                      <span className={`absolute top-0.5 left-0.5 w-5 h-5 rounded-full bg-white shadow-sm transition-transform ${(settings as any)[t.key] ? 'translate-x-5' : ''}`} />
                    </button>
                  </label>
                ))}
              </div>
