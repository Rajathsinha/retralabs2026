...
async function getShiprocketToken(): Promise<string | null> {
  const email = (process.env.SHIPROCKET_EMAIL || process.env.VITE_SHIPROCKET_EMAIL || 'orders@retralabs.in').trim();
  const password = (process.env.SHIPROCKET_PASSWORD || process.env.VITE_SHIPROCKET_PASSWORD || 'MRqH&jLw!dfh$lqjTE#qouoXZP*sXy2G').trim();
  if (!email || !password) {
